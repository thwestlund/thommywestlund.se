...
<script lang="ts">
  import type {
    ...
    Board,
    WinningLine,
  } from "$lib/assets/models/iTicTacToe";
  ...
  let isDraw = $derived.by(
    (): boolean => !winner && board.every((cell) => cell !== null),
  );
...
<main class="flex min-h-screen flex-col items-center gap-8 bg-gray-100 p-8">
  ...
  <div class="text-2xl font-semibold text-gray-700">
    ...
    {:else}
      Aktiv spelare: <span class={getPlayerColor(currentPlayer)}
        >{currentPlayer}</span
      >
    {/if}
  </div>
  ...
  <button
    class="rounded-lg bg-blue-600 px-6 py-3 text-lg font-semibold
